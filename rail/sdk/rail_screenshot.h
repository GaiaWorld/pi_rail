...
class IRailScreenshot : public IRailComponent {
  public:
    ...
    // set accessible users to the screenshot.
    // clear accessible users if the size is zero
    // failed and return false if a rail id is invalid.
    virtual bool SetUsers(const RailArray<RailID>& users) = 0;

    // associate publish files to the screenshot
    // clear accessible published files if the size is zero
    // failed and return false if a work id is invalid.
